...
const MessageForm = ({ reason, setReason }: { reason: string, setReason: React.Dispatch<React.SetStateAction<string | undefined>> }) => {
  ...
  const isReasonPedido = reason === 'pedido'
  ...
  return (
    ...
    <button onClick={() => { setReason('') }} className='lg:pt-10 pt-3 lg:pr-4 w-full flex justify-end '><img src="x.webp" className='lg:h-4' alt="x icon" /></button>
    <h2 className='text-[#f5f5f5] font-extrabold lg:pb-12 lg:pt-10 pt-3 pb-5 w-full lg:text-7xl text-[1.375rem]'>{isReasonPedido ? 'QUEREMOS ORAR CON VOS!' : 'CELEBREMOS JUNTOS!'}</h2>
    <form ref={formRef} action={async (formData) => {
      ...
      <textarea aria-label='content-input' className='lg:h-[300px] h-52 lg:pt-14 px-3 pt-4 text-clip lg:rounded-[50px] rounded-[20px] text-black bg-[#f5f5f5] border-2 border-[#f5f5f5] border-solid focus-visible:outline-none' name='content' placeholder={`Escribe tu ${isReasonPedido ? `${reason} de oracion` : `${reason}`} aqui...`} onChange={() => { setAppStatus(APP_STATUS.TYPING) }} required/>
      <div className='w-full flex justify-center'>
      <button className={`text-[#f5f5f5] flex text-center justify-center w-full lg:text-4xl text-2xl lg:rounded-[50px] rounded-[30px] lg:py-5 py-2 bg-[#EF8100] lg:w-[30rem]  ${appStatus === APP_STATUS.IDLE ? 'pointer-events-none' : ''} `}
