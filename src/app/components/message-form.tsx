...
const MessageForm = ({ motif, setMotif }: { motif: string, setMotif: React.Dispatch<React.SetStateAction<string>> }) => {
  ...
  return (
    ...
    <button onClick={() => { setMotif('') }} className='text-black text-6xl font-bold pt-6 w-full flex justify-end mr-24'>x</button>
    <h2 className='text-black font-extrabold pb-12 mt-14 w-full text-7xl'>QUEREMOS ORAR POR VOS!</h2>
    <form ref={formRef} action={async (formData) => {
      ...
      <button className={`text-[#f5f5f5] flex text-center justify-center w-1/3 text-5xl rounded-[40px] py-6   ${appStatus === APP_STATUS.IDLE || appStatus === APP_STATUS.TYPING ? 'pointer-events-none bg-black/75' : 'bg-black '} `}
