...
const MessageForm = ({ reason, setReason }: { reason: string, setReason: React.Dispatch<React.SetStateAction<string | undefined>> }) => {
  ...
  return (
    ...
    <h2 className='text-[#f5f5f5] font-extrabold lg:pb-12 lg:pt-7 pt-1 pb-5 w-full lg:text-7xl text-[1.25rem]'>{isReasonPedido === 1 ? 'QUEREMOS ORAR CON VOS' : isReasonPedido === 2 ? 'CELEBREMOS JUNTOS' : 'ACTUALIZACIÓN DE PEDIDO'}</h2>
    <form ref={formRef} onSubmit={() => {
      ...
      <textarea aria-label='content-input' className='lg:h-[300px] h-44 lg:pt-14 px-3 pt-2 text-clip lg:rounded-[50px] rounded-[20px] text-black bg-[#f5f5f5] border-2 border-[#f5f5f5] border-solid focus-visible:outline-none resize-none' name='content' placeholder={`Escribe ${isReasonPedido === 1 ? `tu ${reason} de oracion` : isReasonPedido === 3 ? `la ${reason} de tu pedido` : `tu ${reason}`} aqui...`} onChange={() => { setAppStatus(APP_STATUS.TYPING) }} required/>
      <div className='w-full flex justify-center'>
      <button className={`text-[#f5f5f5] flex text-center justify-center w-full lg:text-4xl text-[1.25rem] lg:rounded-[50px] rounded-[30px] lg:py-5 py-2 bg-[#EF8100] lg:w-[30rem]  ${appStatus === APP_STATUS.IDLE || appStatus === APP_STATUS.SENDING ? 'pointer-events-none' : ''}`}
      disabled={isDisabled}>
