...
const MessageForm = ({ reason, setReason }: { reason: string, setReason: React.Dispatch<React.SetStateAction<string | undefined>> }) => {
  ...
  return (
    ...
    <button onClick={() => { setReason('') }} className='text-black text-6xl font-bold pt-4 w-full flex justify-end mr-24'>x</button>
    ...
    <form ref={formRef} action={async (formData) => {
      ...
      <textarea aria-label='content-input' className='h-[300px] pt-14 px-12 text-clip rounded-[90px] bg-black border-2 border-[#f5f5f5] border-solid focus-visible:outline-none' name='content' placeholder={`Escribe tu ${reason}`} onChange={() => { setAppStatus(APP_STATUS.TYPING) }} required/>
